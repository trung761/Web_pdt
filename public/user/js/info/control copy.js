        ...
        $.ajax({
            ...
            success: function (response) {
                if(response == 'check_false'){
                    toastr.warning("Thí sinh chưa tải hình chứng minh nhân dân hoặc Bằng tốt nghiệp THPT")
                }else{
                    if(response > 0 ){
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if(response == "del"){
                            ...
                            if(response == 'check_fasle'){
                                toastr.warning("Chưa tải minh chứng đối tượng ưu tiên")
                            }else{
                                if(response == 1){
                                    toastr.success("Cập nhật thành công")
                                    loadPriority_Policy()
...
function id_card_users_img(id,type){
    $('#open_img_policy').val('');
    $('#crop_policy').attr('id-data',id)
    $('#crop_policy').attr('type',type)
    ...
    $.ajax({
        ...
        success: function (response) {
            if(response.length > 0){
                $('#name_user').val(response[0].name_user)
                $('#birth_user').val(response[0].birth_user)
                $('#id_place_user').val(response[0].id_place_user)
                $('#userImg').attr('src',response[0].link_img_user)
                if(response[0].sex_user == 1){
                    $('#female_user').attr('checked','checked')
                }else{
                    $('#male_user').attr('checked','checked')
                }
                $('#emailsc_user').val(response[0].emailsc_user)
                $('#phonesc_user').val(response[0].phonesc_user)
                $('#address_user').val(response[0].address_user)
                $('#graduation_year_user').val(response[0].graduation_year_user)
                var today = new Date();
                if(response[0].graduation_year_user < today.getFullYear()){
                    $('#graduation_year_user_img').show()
                }else{
                    $('#graduation_year_user_img').hide()
                }
            }else{
                $('#birth_user').val('2004-01-01')
            }
        }
    });
}
...
// image_crop_policy = $('#resizer-policy').croppie({
//     enableExif: true,
//     viewport: {
//         width:350,
//         height:500,
...
//         var reader = new FileReader();
//         reader.onload = function (event) {
//             image_crop_policy.croppie('bind', {
//             url: event.target.result
//             })
//         }
//         reader.readAsDataURL(this.files[0]);
...
// // /Add Img
// $('#crop_policy').click(function(event){
//     var a = $(this).attr('id-data')
//     var b = $(this).attr('type')
//     // alert($(this).attr('type'))
//     image_crop_policy.croppie('result', {
//         type: 'canvas',
//         size: 'viewport'
...
// $('#crop_policy_close').click(function(event){
//     $('#modal_policy').hide('slow');
// })
