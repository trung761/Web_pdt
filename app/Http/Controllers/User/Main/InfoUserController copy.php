...
function check_id_card($year){
    ...
    public function add_infoUser(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if($this->check_id_card($request->input('graduation_year_user')) == 1){
    ...
    public function Priority_Policy(){
        ...
        foreach ($list as $key => $value) {
            $html .= '<i   class="fa fa-paperclip" onclick = "policy('.$value->id.',1)" style="color: #007bff" aria-hidden="true"></i>'.$value->name_list.";&nbsp;&nbsp;&nbsp";
            $html = rtrim($html, ";   ");
        }
        }else{
            $html = "";
    ...
    public function changePriority_Policy($id){
        ...
        if(count($list) >0){
            $html = "Tải minh chứng:&nbsp;&nbsp;&nbsp";
        foreach ($list as $key => $value) {
            // $html .= '<i class="fa fa-paperclip" id = "policy_attr" onclick = "policy('.$value->id.',1)" style="color: #007bff" aria-hidden="true"></i>'.$value->name_list.";&nbsp;&nbsp;&nbsp";
        ...
        if($this ->check_reg() == 1){
            ...
            if($act == 1){
                $result = array(
                    'policys' =>$policys[0]->note_policy_user,
                    'html' => $html,
                    'act' => $act
                );
                return $result;
    ...
    public function addPriority_policy(Request $request){
        if($request->input('id') == 0){
            ...
            $check_list= DB::table('l_policy_users_list')
            ->where('id_policy_users',$request->input('id'))
            ->get();

            if(count($check) > 0){
                $i = 0;
                foreach ($check_list as $key => $value1) {
                    foreach ($check as $key => $value2) {
                        if($value1->id == $value2->id_img)
                        $i++;
                    }
                }
                if(count($check_list)==$i){
    ...
    public function crop_policy(Request $request){
        ...
        $path = '/images/hocba'.'/'.Auth::guard('user')->user()->id.'/policy_'.$request->input('type').'_'.$request->input('id').'_'.$fileName;
        $data =  $request->input('img');
        list($type, $data) = explode(';', $data);
        list(, $data)      = explode(',', $data);
        $data = base64_decode($data);
        $storage = Storage::disk('local');
        $storage->put('/hocba'.'/'.Auth::guard('user')->user()->id.'/policy_'.$request->input('type').'_'.$request->input('id').'_'.$fileName, $data, 'public');
